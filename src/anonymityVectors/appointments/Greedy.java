...
public class Greedy extends Appointment {
	...
	public static boolean findSlots(int ipos, Integer[] f, int[] c, int[][] a) {
		...
		boolean[] selected = new boolean[c.length]; // characteristic function
													// indicating if a resource
													// has been selected
		int[] iselected = new int[c.length]; // characteristic function
												// indicating if a resource has
												// been selected
		...
		if (nfound == c.length && sofar < q) {
			...
			// Phase 2: decrease the capacity and write-down the appointment
			boolean[] sel = new boolean[nfound]; // to mark the already selected
													// resource indices
			for (int i = 0; i < nfound && q > 0; i++) {
				...
				if (c[iselected[imin]] > q) {
					// the minimum covers the remaining population...then it is
					// better to take the greatest value
